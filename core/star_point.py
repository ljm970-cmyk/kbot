...
PRICE_GUARD_PCT = 0.25


#: 기준을 25% 로 두는 이유 (실측 2026-09-23 반영)
#:
#: 별% 는 T=0 에서 최대 20%(SOXL 40분할)/15%(TQQQ) 다. 사이클 초반에는
#: 현재가와 평단이 비슷해서 별지점이 현재가보다 19~20% 위에 놓인다.
#: 기준이 18% 면 이 정상 상태에서 매일 대체주문이 발동해, 별지점·평단
#: 2분할이 통째로 현재가 기준 단건으로 바뀐다. 그러면 방법론이라면
#: 사지 않았을 가격에도 매수가 들어간다.
#:   실제 사례: 평단 151.95, 현재가 150.86, T=1 → 별지점 180.82 (+19.85%)
#:              → 2주 @173.49 단건으로 대체됨 (별지점 1주 + 평단 2주 대신)
#:
#: 25% 는 정상 구간(최대 20%)을 넘어서므로, 평단이 현재가보다 한참 위인
#: 진짜 폭락에서만 발동한다. 키움은 현재가 −29% LOC 를 받아줬고(실측)
#: +15% 도 접수됐다. 실제 거부 한계는 아직 모르며, 실시간 주문이라
#: 거부되면 접수 즉시 알 수 있다.
